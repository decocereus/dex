...
import type { ContextMenuItem } from "@dex/contracts";
import { RotatingFileSink } from "@dex/shared/logging";
...
const BASE_DIR = process.env.DEX_HOME?.trim() || Path.join(OS.homedir(), ".dex");
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const DESKTOP_SETTINGS_PATH = Path.join(STATE_DIR, "desktop-settings.json");
const CLIENT_SETTINGS_PATH = Path.join(STATE_DIR, "client-settings.json");
const SAVED_ENVIRONMENT_REGISTRY_PATH = Path.join(STATE_DIR, "saved-environments.json");
const DESKTOP_SCHEME = "dex";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? "dex (Dev)" : "dex (Alpha)";
const APP_USER_MODEL_ID = isDevelopment ? "com.amartyasing.dex.dev" : "com.amartyasing.dex";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "dex-dev.desktop" : "dex.desktop";
const LINUX_WM_CLASS = isDevelopment ? "dex-dev" : "dex";
const USER_DATA_DIR_NAME = isDevelopment ? "dex-dev" : "dex";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "dex (Dev)" : "dex (Alpha)";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.DEX_PORT;
  delete env.DEX_MODE;
  delete env.DEX_NO_BROWSER;
  delete env.DEX_HOST;
  delete env.DEX_DESKTOP_WS_URL;
  delete env.DEX_DESKTOP_LAN_ACCESS;
  delete env.DEX_DESKTOP_LAN_HOST;
  return env;
}
...
function resolveAdvertisedHostOverride(): string | undefined {
  const override = process.env.DEX_DESKTOP_LAN_HOST?.trim();
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { dexCommitHash?: unknown };
    return normalizeCommitHash(parsed.dexCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.DEX_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("dex failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const hasUpdateFeedConfig =
    readAppUpdateYml() !== null || Boolean(process.env.DEX_DESKTOP_MOCK_UPDATES);
  const disabledReason = getAutoUpdateDisabledReason({
    isDevelopment,
    isPackaged: app.isPackaged,
    platform: process.platform,
    appImage: process.env.APPIMAGE,
    disabledByEnv: process.env.DEX_DISABLE_AUTO_UPDATE === "1",
    hasUpdateFeedConfig,
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `dex ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/dex (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`dex`). If the legacy
...
function configureAutoUpdater(): void {
  const githubToken =
    process.env.DEX_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
  ...
  if (process.env.DEX_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.DEX_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
...
async function bootstrap(): Promise<void> {
  ...
  const configuredBackendPort = resolveConfiguredDesktopBackendPort(process.env.DEX_PORT);
  if (isDevelopment && configuredBackendPort === undefined) {
    throw new Error("DEX_PORT is required in desktop development.");
  }
