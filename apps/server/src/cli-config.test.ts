...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  const openBootstrapFd = Effect.fn(function* (payload: Record<string, unknown>) {
    ...
    const filePath = yield* fs.makeTempFileScoped({ prefix: "dex-bootstrap-", suffix: ".ndjson" });
  ...
  it.effect("falls back to effect/config values when flags are omitted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = join(os.tmpdir(), "dex-cli-config-env-base");
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  DEX_NO_BROWSER: "true",
                  DEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  DEX_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("uses CLI flags when provided", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(os.tmpdir(), "dex-cli-config-flags-base");
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  DEX_PORT: "4001",
                  DEX_HOST: "0.0.0.0",
                  DEX_HOME: join(os.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  DEX_NO_BROWSER: "false",
                  DEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  DEX_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("preserves explicit false CLI boolean flags over env and bootstrap values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(os.tmpdir(), "dex-cli-config-false-flags");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  DEX_BOOTSTRAP_FD: String(fd),
                  DEX_NO_BROWSER: "true",
                  DEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  DEX_LOG_WS_EVENTS: "true",
                },
              }),
            ),
  ...
  it.effect("uses bootstrap envelope values as fallbacks when flags and env are absent", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = "/tmp/dex-bootstrap-home";
      const fd = yield* openBootstrapFd({
        mode: "desktop",
        port: 4888,
        host: "127.0.0.2",
  ...
  it.effect("creates derived runtime directories during config resolution", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "dex-cli-config-dirs-" });
  ...
  it.effect("applies flag then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(os.tmpdir(), "dex-cli-config-env-wins");
      const fd = yield* openBootstrapFd({
        mode: "desktop",
        port: 4888,
        host: "127.0.0.2",
        t3Home: "/tmp/dex-bootstrap-home",
        devUrl: "http://127.0.0.1:5173",
        noBrowser: false,
        autoBootstrapProjectFromCwd: false,
        logWebSocketEvents: false,
  ...
  it.effect("falls back to persisted observability settings when env vars are absent", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "dex-cli-config-settings-" });
  ...
  it.effect("forces noBrowser and disables auto-bootstrap for headless startup presentation", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(os.tmpdir(), "dex-cli-config-headless-base");
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
