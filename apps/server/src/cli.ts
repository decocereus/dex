...
import { parsePersistedServerObservabilitySettings } from "@dex/shared/serverSettings";
import {
  AuthSessionId,
  CommandId,
...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to DEX_HOME)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to DEX_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("DEX_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.logLevel("DEX_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.boolean("DEX_TRACE_TIMING_ENABLED").pipe(Config.withDefault(true)),
  traceFile: Config.string("DEX_TRACE_FILE").pipe(Config.option, Config.map(Option.getOrUndefined)),
  traceMaxBytes: Config.int("DEX_TRACE_MAX_BYTES").pipe(Config.withDefault(10 * 1024 * 1024)),
  traceMaxFiles: Config.int("DEX_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.int("DEX_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(200)),
  otlpTracesUrl: Config.string("DEX_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.string("DEX_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpExportIntervalMs: Config.int("DEX_OTLP_EXPORT_INTERVAL_MS").pipe(Config.withDefault(10_000)),
  otlpServiceName: Config.string("DEX_OTLP_SERVICE_NAME").pipe(Config.withDefault("dex-server")),
  mode: Config.schema(RuntimeMode, "DEX_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.port("DEX_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("DEX_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  t3Home: Config.string("DEX_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("DEX_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("DEX_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("DEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("DEX_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
...
const startCommand = Command.make("start", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription("Run the dex server."),
  Command.withHandler((flags) => runServerCommand(flags)),
);

const serveCommand = Command.make("serve", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription(
    "Run the dex server without opening a browser and print headless pairing details.",
  ),
  Command.withHandler((flags) =>
...
export const cli = Command.make("dex", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription("Run the dex server."),
  Command.withHandler((flags) => runServerCommand(flags)),
