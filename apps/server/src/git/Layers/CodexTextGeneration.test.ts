...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  ...
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "dex-codex-text-generation-test-",
    }),
  ),
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__DEX_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__DEX_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "dex-codex-text-" });
