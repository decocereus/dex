...
import { GitManagerError } from "@dex/contracts";
import {
  GitManager,
...
import type { GitManagerServiceError } from "@dex/contracts";
import {
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `dex/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `dex-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
