...
- Publishes the CLI package (`apps/server`, npm package `dex`) with OIDC trusted publishing.
...
- Repository slug source:
  - `DEX_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `DEX_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `dex` (or rename package first if needed).
