...
dex has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.dex/userdata/logs/server.trace.ndjson`).
...
```bash
export DEX_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export DEX_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export DEX_OTLP_SERVICE_NAME=dex-local
```

Optional:

```bash
export DEX_TRACE_MIN_LEVEL=Info
export DEX_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `DEX_OTLP_*`.

macOS app bundle example:

```bash
DEX_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
DEX_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
DEX_OTLP_SERVICE_NAME=dex-desktop \
...
```bash
tail -f "$DEX_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$DEX_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$DEX_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `dex-local`, `dex-dev`, or `dex-desktop`
...
- `DEX_OTLP_TRACES_URL` was not set
...
- `DEX_TRACE_FILE`: override trace file path
- `DEX_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `DEX_TRACE_MAX_FILES`: rotated file count, default `10`
- `DEX_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `DEX_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `DEX_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `DEX_OTLP_TRACES_URL`: OTLP trace endpoint
- `DEX_OTLP_METRICS_URL`: OTLP metric endpoint
- `DEX_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `DEX_OTLP_SERVICE_NAME`: service name, default `dex-server`
