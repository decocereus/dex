...
Use this when you want to connect to a dex server from another device such as a phone, tablet, or separate desktop app.
...
Run the server with `dex serve`.

```bash
npx dex serve --host "$(tailscale ip -4)"
```

`dex serve` starts the server without opening a browser and prints:
...
Use `dex serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

> Note
> The GUIs do not currently support adding projects on remote environments.
> For now, use `dex project ...` on the server machine instead.
...
1. `dex serve` issues a one-time owner pairing token.
...
Use `dex auth` to manage access after the initial pairing flow.
...
Use `dex auth --help` and the nested subcommand help pages for the full reference.
...
- Use `dex auth` to revoke credentials or sessions you no longer trust.
