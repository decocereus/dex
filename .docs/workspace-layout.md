...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `dex` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@dex/shared/git`, `@dex/shared/DrainableWorker`) — no barrel index.
