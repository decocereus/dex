...
- Dev commands default `DEX_STATE_DIR` to `~/.dex/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.dex-2`
...
- Desktop production windows load the bundled UI from `dex://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `dex` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `DEX_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `DEX_DEV_INSTANCE`)
- Example: `DEX_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `DEX_PORT_OFFSET` to a numeric offset.
