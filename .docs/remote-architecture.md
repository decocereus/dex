...
This document describes the target architecture for first-class remote environments in dex.
...
- Keep the dex server as the execution boundary.
...
dex already has a clean runtime boundary: the client talks to a dex server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one dex server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one dex server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the dex runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running dex server instance.
...
How does the client speak WebSocket to a dex server?
...
- best fit for hosted or self-managed remote dex deployments
...
For dex, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
- reach a machine
- probe it
- launch or reuse a remote dex server
...
How does a dex server come to exist on the target machine?
...
The user or operator already runs dex on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in dex:
...
The recommended dex flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies dex availability.
3. Desktop launches or reuses a remote dex server.
...
This is the inverse of remote launch: a local dex server is already running, and the client publishes it through a tunnel.
...
- A manually hosted dex server might be reached through direct `wss`.
...
dex already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
dex should not copy that part.

dex already has the right runtime boundary:

- one dex server per environment
- ordinary HTTP/WebSocket between client and environment

So dex should borrow Zed's launch discipline, not its transport protocol.
